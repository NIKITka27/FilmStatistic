...
class ReviewController {
    ...
    fun updateReview(@PathVariable("id") id: Long, model: Model): ModelAndView {
        val review: Review = reviewRepository.findById(id).orElseThrow() {
    ...
    fun updateReview(@PathVariable("id") id: Long, @Validated review: Review, result: BindingResult, model: Model): ModelAndView {
        val modelAndView: ModelAndView = ModelAndView()
        if (result.hasErrors()) {
            model.addAttribute("review", review)
            modelAndView.viewName = "updateReview"
